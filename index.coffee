...
module.exports = class NodeNuget
  ...
  @push: (file, callback) ->
    ...
    # run push command
    queue.defer (callback) ->
      return callback(new Error "Failed to push file: #{file.path}") if runCommand('push', [file_path, '-Source', 'https://www.nuget.org/api/v2/package', '-NonInteractive']).code isnt 0
      callback()
