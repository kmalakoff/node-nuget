...
const runCommand = function (command, args, callback) {
...
class NodeNuget {
    ...
    getFile(file, function (err, file) {
      ...
      file.pipe(
        es.wait(function (err, data) {
          ...
          runCommand('pack', [file.path], function (err) {
            if (err) return callback(err);
  ...
  static push(file, callback) {
    ...
    queue.defer(function (callback) {
      if (!file.pipe) return callback(null, (filePath = file));
      if (fs.existsSync((filePath = file.path))) return callback(); // use if exists on disk
